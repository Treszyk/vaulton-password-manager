...
@Component({
  ...
  template: `
    <div
      class="fixed inset-0 z-[100] overflow-y-auto bg-black/60 backdrop-blur-md"
      [class.animate-fade-in]="!isClosing()"
      [class.animate-fade-out]="isClosing()"
    >
      <div
        class="min-h-full w-full flex items-end md:items-center justify-center p-0 md:p-4 pointer-events-none"
        (click)="triggerClose()"
      >
        <div
          class="w-full md:max-w-lg bg-transparent md:bg-vault-black border-none md:border border-zinc-600 rounded-t-[2rem] md:rounded-3xl shadow-none md:shadow-2xl overflow-hidden flex flex-col max-h-[98dvh] md:max-h-[85vh] relative pointer-events-auto"
